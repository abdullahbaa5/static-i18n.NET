...
public class I18NextBuilder
{
    ...
    /// <summary>
    ///     Sets the global default locale file format used by I18Next to resolve translations.
    /// </summary>
    /// <param name="fileFormat">The default locale file format.</param>
    /// <returns>The current I18Next builder instance.</returns>
    /// <exception cref="ArgumentException">If the provided default locale file format value is null or empty.</exception>
    public I18NextBuilder UseDefaultLocaleFileFormat(string fileFormat)
    {
        if (string.IsNullOrEmpty(fileFormat))
            throw new ArgumentException("locale file format cannot be null or empty.", nameof(fileFormat));

        Services.Configure<I18NextOptions>(options => options.DefaultFileFormat = fileFormat);
